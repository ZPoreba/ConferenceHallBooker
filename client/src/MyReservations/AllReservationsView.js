...
class AllReservationsView extends Component {
    ...
    getAllReservations () {
        myReservationsService.getAllReservations().then((resp) => {
    ...
    parseDate() {
        ...
        for (let reservation of this.state.userReservations){
            ...
            newData.push({
              ...
              content:  "User id: " + reservation.user_id + ", "
                        + "Total: " + reservation.price + " zł, "
    ...
    setReservationStatus = (id, status) => {
        myReservationsService.setReservationStatus({id: id, status: status}).then((resp) => {
            alert(`Status of reservation has been changed to: ${status}`);
            this.getAllReservations();
        });
    }; 
                ...
                <List
                    ...
                    renderItem={item => (
                            ...
                            actions={[
                                ...
                                <CheckOutlined style={{color: "green", fontSize: "x-large"}} onClick={(e) => {this.setReservationStatus(item.id, 'accepted')}}  />,
                                <CloseOutlined style={{color: "red", fontSize: "x-large"}} onClick={(e) => {this.setReservationStatus(item.id, 'rejected')}} />
                              ]}
                            extra={
                                <img
                                width={200}
...
export default withRouter(AllReservationsView);